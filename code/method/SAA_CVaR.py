...
class SAA_CVaR(strategy):
    ...
    def optimize(self, train_return, test_return, 
                 weight_pre):#balance between cvar and mean
        m = Model('SAA_CVaR')
    ...
    def optimize_adj(self, train_return, test_return, weight_pre):
        m = Model('SAA_CVaR')
    ...
    def rolling(self):
        ...
        while i < num_of_sample - num_of_train:
            ...
            [self.weight, self.return_array[i:i + self.rolling_day]] = self.optimize(train_return, test_return, self.weight)
            if type(self.adj_level) != bool:
                [self.weight, self.return_array[i:i + self.rolling_day]] = self.optimize_adj(train_return, test_return, self.weight)
