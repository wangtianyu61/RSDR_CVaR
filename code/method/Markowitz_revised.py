...
class Markowitz(strategy, strategy_cluster, resample):
    ...
    #Create a Model
        ...
        if robust_sign == 0:
        #Set the objective :
            obj = self.risk_aversion*(S.dot(weight).dot(weight)) +tran_cost_p*np.sum(weight_dif)- np.dot(mu,weight)
            ...
            obj = self.gamma2*self.risk_aversion*(S.dot(weight).dot(weight)) +tran_cost_p*np.sum(weight_dif)-np.dot(mu,weight) + math.sqrt(self.gamma1)*covar_bound
            m.setObjective(obj, GRB.MINIMIZE)
    ...
    def rolling(self, shortsale_sign, cluster_sign, cluster_number, robust_sign = 0):
        i = 0
