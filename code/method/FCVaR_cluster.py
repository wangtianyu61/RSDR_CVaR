...
class FCVaR_cluster(strategy, strategy_cluster):
    ...
    def __init__(self, df_select, df_train, rolling_day, portfolio_number, df_factor,
                         cluster_sign, cluster_number, method_name, 
                         mean_target = False, adj_level = False, 
                         hmm_state_estimate = False): 
    #control the way to generate regimes and param probability
    ...
    def optimize(self, cluster_number, cluster_freq, mean_info, cov_info, 
                 test_return, epsilon,weight_pre,tran_cost_p, shortsale_sign, train_return_mean,
                 trade_off = 0):#cvar and mean balance
        # Create a new model
        #data preprocessing    
        ...
        #print(mean_info.loc[0], port_num,  test_return)
        ## Set the objective: 
        obj = v
        for i in range(cluster_number):
            obj = obj + cluster_freq[i]*(covar_mean[i] + tran_cost_p*np.sum(weight_dif)- (1 + trade_off)*np.dot(mean_info.iloc[i], weight) - v)/(2*epsilon) 
        # Set the constraints:
...
#        print(covar_mean)
    ...
    def cross_validation(self, train_return, fold_num):
        right_param = 0
        for serial_number in range(1, fold_num + 1):
            print("serial", serial_number)
            [train_train, train_validation] = self.divide(train_return, fold_num, serial_number)
            train_return_mean = np.array(train_train.mean())
            #currently just use clustering for reference
            ...
            temp_SR = np.zeros(len(tradeoff_list))
            for i in range(len(tradeoff_list)):
                weight = np.ones(self.portfolio_number)/self.portfolio_number
                [weight, return_array] = self.optimize(self.cluster_number, cluster_freq, mean_info, cov_info, train_validation,
                                                                                        epsilon, weight, tran_cost_p, shortsale_sign, train_return_mean, tradeoff_list[i])            
                temp_SR[i] = np.mean(return_array)/np.std(return_array)
            right_param += tradeoff_list[np.argmax(temp_SR)]
        return right_param/fold_num
    ...
    def rolling(self, shortsale_sign):
        ...
        while i < num_of_sample - num_of_train:
            ...
            print(np.array(train_return).shape)
            ...
            #tradeoff = self.cross_validation(train_return, fold_number)
            if self.cluster_sign == 1:
                ...
                [cluster_freq, mean_info, cov_info] = self.return_cluster(train_return,self.column_name,self.cluster_number, self.hmm_state_estimate)
            ...
            [self.weight, self.return_array[i:i + self.rolling_day]] = self.optimize(self.cluster_number, cluster_freq, mean_info, cov_info, test_return,
                                                                                        epsilon, self.weight, tran_cost_p, shortsale_sign, train_return_mean, tradeoff)
            if type(self.adj_level) != bool:
