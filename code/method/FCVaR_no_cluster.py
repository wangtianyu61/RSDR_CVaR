...
class F_CVaR(strategy):
    ...
    def optimize(self, train_return_mean,train_return_covar,test_return,
                 weight_pre,tran_cost_p, epsilon, shortsale_sign,
                 trade_off = 0):#balance between 
        ...
        obj = v + (covar - (1 + trade_off)*np.dot(mu,weight) - v + tran_cost_p*np.sum(weight_dif))/(2*epsilon) 
    ...
    def rolling(self, shortsale_sign):
        ...
        while i < num_of_sample - num_of_train:   
            ...
            [self.weight, self.return_array[i:i + self.rolling_day]] = self.optimize(train_return_mean,train_return_covar,test_return,self.weight,tran_cost_p, epsilon, shortsale_sign, tradeoff)
            if type(self.adj_level) != bool:
